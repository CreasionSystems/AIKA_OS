...
describe("createAikaApi: channel への委譲と素通し", () => {
  ...
  it("planCode は goal を引数に内部 channel へ委譲する", async () => {
    const { invoke, calls } = makeInvokeSpy();
    const api = createAikaApi(invoke);
    await api.planCode("add feature");
    expect(calls[0]?.channel).toBe(IPC_CHANNELS.planCode);
    expect(calls[0]?.args).toEqual(["add feature"]);
  });

  it("executeCode は引数なしで内部 channel へ委譲する", async () => {
    const { invoke, calls } = makeInvokeSpy();
    const api = createAikaApi(invoke);
    await api.executeCode();
    expect(calls[0]?.channel).toBe(IPC_CHANNELS.executeCode);
    expect(calls[0]?.args).toEqual([]);
  });

  it("verifyCode は引数なしで内部 channel へ委譲する", async () => {
    const { invoke, calls } = makeInvokeSpy();
    const api = createAikaApi(invoke);
    await api.verifyCode();
    expect(calls[0]?.channel).toBe(IPC_CHANNELS.verifyCode);
    expect(calls[0]?.args).toEqual([]);
  });

  it("rewindCode は引数なしで内部 channel へ委譲する", async () => {
    const { invoke, calls } = makeInvokeSpy();
    const api = createAikaApi(invoke);
    await api.rewindCode();
    expect(calls[0]?.channel).toBe(IPC_CHANNELS.rewindCode);
    expect(calls[0]?.args).toEqual([]);
  });

  it("listJobs は引数なしで内部 channel へ委譲する", async () => {
    const { invoke, calls } = makeInvokeSpy();
    const api = createAikaApi(invoke);
    await api.listJobs();
